...
sql = "INSERT IGNORE INTO  cryptos (ID, Name) VALUES (%s, %s)"
...
def get_tokens(signature: Signature, RaydiumLPV4: Pubkey) -> None:
    ...
    if transaction.value is None:
        logging.error(f"No value found for transaction {signature}.")
        return
    #print("gigi", type(signature), signature)
    val = (str(signature), "")
    ...
    for instruction in filtred_instuctions:
        ...
        print_table(tokens)
        counter = counter + 1
        print(f"True, {counter} https://solscan.io/tx/{signature}")
